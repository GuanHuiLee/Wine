...
public class ArticlePresenter extends ArticleContract.Presenter {
    ...
    @Override
    public void getUserArticleList(Map<String, Object> params) {
        model.getUserArticleList(params, new MyCallBack<BaseResult<String>>() {
            @Override
            public void onSuc(Response<BaseResult<String>> response) {
                if (isAttach) {
                    BaseResult<String> body = response.body();
                    if (body.getCode() == 200) {
                        List<UserArticleBean> list = new Gson().fromJson(body.getData(), new TypeToken<List<UserArticleBean>>() {
                        }.getType());
                        view.showUserArticleList(list);
                    } else view.showError(body.getMsg());
                    view.hideProgress();
                }
            }
